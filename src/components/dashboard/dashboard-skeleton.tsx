...
function Bar({ className }: { className?: string }) {
  return (
    <div
      className={`animate-pulse rounded-md bg-linear-to-r from-white/7 to-white/12 ${className ?? ''}`}
    />
  );
}
...
export function DashboardSkeleton() {
  return (
    ...
    >
      {/* DashboardSummary: section mb-10 → h2 mb-4 → grid gap-4 */}
      <div className="mb-10">
      ...
      {/* UpcomingPayments: section mb-3 mt-2 → h2 mb-3 → ul gap-3 */}
      <section className="mb-4 mt-2">
        <Bar className="mb-3 h-7 w-56" />
        <ul className="flex flex-col gap-3">
          {[0, 1, 2].map((i) => (
            <li key={i}>
              <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-white/3 px-4 py-4">
                <Bar className="h-9 w-9 shrink-0 rounded-full" />
                <Bar className="h-5 min-w-0 flex-1 max-w-50" />
                <Bar className="hidden h-4 w-16 sm:block" />
                <Bar className="hidden h-4 w-14 sm:block" />
                <Bar className="hidden h-4 w-20 sm:block md:block" />
              </div>
            </li>
          ))}
        </ul>
      ...
      {/* Charts: section mt-0 → h2 mb-3 → grid gap-6 */}
      <section className="mt-0">
        <Bar className="mb-3 h-7 w-24" />
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="min-h-70 rounded-xl border border-white/10 bg-white/3 p-4">
